...
void ScsiController::Selection()
{
    ...
    if (!IsSelection()) {
        ...
        if (!GetLunCount()) {
            LOGDEBUG("Abort if there is no LUN for this controller")
            return;
        }
...
{
    stringstream s;
    s << "Controller is executing " << command_mapping.find(GetOpcode())->second.second << ", CDB $" << setfill('0')
      << hex;
...
void ScsiController::Error(sense_key sense_key, asc asc, status status)
{
    ...
    if (sense_key != sense_key::NO_SENSE || asc != asc::NO_ADDITIONAL_SENSE_INFORMATION) {
        stringstream s;
        s << setfill('0') << setw(2) << hex << "Error status: Sense Key $" << static_cast<int>(sense_key) << ", ASC $"
          << static_cast<int>(asc);
        logger.Debug(s.str());
...
void ScsiController::Receive()
{
    ...
    if (HasValidLength()) {
        ...
        if (uint32_t len = GetBus().ReceiveHandShake(GetBuffer().data() + GetOffset(), GetLength());
            len != GetLength()) {
...
void ScsiController::DataOutNonBlockOriented()
{
    ...
    case scsi_command::eCmdModeSelect10: {
        if (auto device = dynamic_pointer_cast<ModePageDevice>(GetDeviceForLun(GetEffectiveLun())); device != nullptr) {
...
bool ScsiController::XferOutBlockOriented(bool cont)
{
    ...
    default:
        stringstream s;
        s << "Received an unexpected command ($" << setfill('0') << setw(2) << hex << static_cast<int>(GetOpcode())
          << ")";
        logger.Warn(s.str());
        break;
    }
