...
bool SCSIDaynaPort::Init(const unordered_map<string, string>& params)
{
	...
	if(!m_bTapEnable){
		logger.Error("Unable to open the TAP interface");
...
// Not terminating on regular Linux PCs is helpful for testing
	...
	return false;
		...
		logger.Trace("Tap interface created");
	}
...
int SCSIDaynaPort::Read(const vector<int>& cdb, vector<uint8_t>& buf, uint64_t)
{
	...
	logger.Trace("Read maximum length: " + to_string(requested_length));
	...
	while (read_count < MAX_READ_RETRIES) {
		...
		if (rx_packet_size <= 0) {
			logger.Trace("No packet received");
			response->length = 0;
			response->flags = read_data_flags_t::e_no_more_data;
			return DAYNAPORT_READ_HEADER_SZ;
		}

		logger.Trace("Packet Size " + to_string(rx_packet_size) + ", read count: " + to_string(read_count));
...
bool SCSIDaynaPort::WriteBytes(const vector<int>& cdb, vector<uint8_t>& buf, uint32_t)
{
	...
	if (data_format == 0x00) {
		...
		logger.Trace("Transmitted " + to_string(data_length) + " byte(s) (00 format)");
	}
	else if (data_format == 0x80) {
		// The data length is specified in the first 2 bytes of the payload
		data_length = buf[1] + ((static_cast<int>(buf[0]) & 0xff) << 8);
		m_tap.Send(&(buf.data()[4]), data_length);
		logger.Trace("Transmitted " + to_string(data_length) + "byte(s) (80 format)");
	}
	else {
		stringstream s;
...
void SCSIDaynaPort::Read6()
{
	...
	if (GetController()->GetCmd(5) != 0xc0 && GetController()->GetCmd(5) != 0x80) {
		logger.Trace("Control value: " + to_string(GetController()->GetCmd(5)));
		throw scsi_exception(sense_key::ILLEGAL_REQUEST, asc::INVALID_FIELD_IN_CDB);
	}

	logger.Trace("READ(6) command record=" + to_string(record) + ", blocks=" + to_string(GetController()->GetBlocks()));
...
void SCSIDaynaPort::EnableInterface()
{
	if (GetController()->GetCmd(5) & 0x80) {
		if (!m_tap.Enable()) {
			logger.Warn("Unable to enable the DaynaPort Interface");
		...
		logger.Info("The DaynaPort interface has been ENABLED");
	}
	else {
		if (!m_tap.Disable()) {
			logger.Warn("Unable to disable the DaynaPort Interface");

			throw scsi_exception(sense_key::ABORTED_COMMAND);
		}

		logger.Info("The DaynaPort interface has been DISABLED");
	}
