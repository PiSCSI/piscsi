...
def list_files(sub_folder=""):
    path = f"{base_dir}{sub_folder}"
    files_list = []
    # Only list valid file types
    # files = [f for f in file if re.match(valid_file_types, f)]
    files_list.extend(
        [
            (
...
def load_config(file_name, verbose=False):
    import csv
    ...
    with open(file_name) as csv_file:
        ...
        for row in config_reader:
            ...
            if verbose:
                print(f" * Attaching Image: {image_name}")
