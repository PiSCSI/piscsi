...
bool RascsiExecutor::Attach(const CommandContext& context, const PbDeviceDefinition& pb_device, bool dryRun)
{
	...
	auto device = unique_ptr<PrimaryDevice>(device_factory.CreateDevice(controller_manager, type, lun, filename));
	if (device == nullptr) {
		if (type == UNDEFINED) {
	...
	if (!controller_manager.CreateScsiController(id, device.get())) {
