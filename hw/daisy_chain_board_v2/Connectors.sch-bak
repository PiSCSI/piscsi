EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 2 2
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Connector_Generic:Conn_02x25_Odd_Even J3
U 1 1 5FED2F59
P 2950 1700
F 0 "J3" H 3000 275 50  0000 C CNN
F 1 "Conn_02x25_Odd_Even" H 3000 366 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_2x25_P2.54mm_Vertical" H 2950 1700 50  0001 C CNN
F 3 "https://datasheet.lcsc.com/szlcsc/1810221914_BOOMELE-Boom-Precision-Elec-C30006_C30006.pdf" H 2950 1700 50  0001 C CNN
F 4 "C30006" H 2950 1700 50  0001 C CNN "LCSC"
F 5 "Header 50 0.100\"(2.54mm) 2 2.54MM IDC Connectors RoHS" H 2950 1700 50  0001 C CNN "Description"
F 6 "BOOMELE(Boom Precision Elec)" H 2950 1700 50  0001 C CNN "Manufacturer_Name"
F 7 "C30006" H 2950 1700 50  0001 C CNN "Manufacturer_Part_Number"
F 8 "" H 2950 1700 50  0001 C CNN "Mouser Part Number"
F 9 "" H 2950 1700 50  0001 C CNN "Mouser Price/Stock"
	1    2950 1700
	0    -1   -1   0   
$EndComp
Wire Wire Line
	1750 2000 1750 1900
Wire Wire Line
	1750 2000 1850 2000
Wire Wire Line
	1850 2000 1850 1900
Wire Wire Line
	1850 2000 1950 2000
Wire Wire Line
	1950 2000 1950 1900
Connection ~ 1850 2000
Wire Wire Line
	1950 2000 2050 2000
Wire Wire Line
	2050 2000 2050 1900
Connection ~ 1950 2000
Wire Wire Line
	2050 2000 2150 2000
Wire Wire Line
	2150 2000 2150 1900
Connection ~ 2050 2000
Wire Wire Line
	2150 2000 2250 2000
Wire Wire Line
	2250 2000 2250 1900
Connection ~ 2150 2000
Wire Wire Line
	2250 2000 2350 2000
Wire Wire Line
	2350 2000 2350 1900
Connection ~ 2250 2000
Wire Wire Line
	2350 2000 2450 2000
Wire Wire Line
	2450 2000 2450 1900
Connection ~ 2350 2000
Wire Wire Line
	2450 2000 2550 2000
Wire Wire Line
	2550 2000 2550 1900
Connection ~ 2450 2000
Wire Wire Line
	2550 2000 2650 2000
Wire Wire Line
	2650 2000 2650 1900
Connection ~ 2550 2000
Wire Wire Line
	2650 2000 2750 2000
Wire Wire Line
	2750 2000 2750 1900
Connection ~ 2650 2000
Wire Wire Line
	2750 2000 2850 2000
Wire Wire Line
	2850 2000 2850 1900
Connection ~ 2750 2000
Wire Wire Line
	2850 2000 3050 2000
Wire Wire Line
	3050 2000 3050 1900
Connection ~ 2850 2000
Wire Wire Line
	3050 2000 3150 2000
Wire Wire Line
	3150 2000 3150 1900
Connection ~ 3050 2000
Wire Wire Line
	3150 2000 3250 2000
Wire Wire Line
	3250 2000 3250 1900
Connection ~ 3150 2000
Wire Wire Line
	3250 2000 3350 2000
Wire Wire Line
	3350 2000 3350 1900
Connection ~ 3250 2000
Wire Wire Line
	3350 2000 3450 2000
Wire Wire Line
	3450 2000 3450 1900
Connection ~ 3350 2000
Wire Wire Line
	3450 2000 3550 2000
Wire Wire Line
	3550 2000 3550 1900
Connection ~ 3450 2000
Wire Wire Line
	3550 2000 3650 2000
Wire Wire Line
	3650 2000 3650 1900
Connection ~ 3550 2000
Wire Wire Line
	3650 2000 3750 2000
Wire Wire Line
	3750 2000 3750 1900
Connection ~ 3650 2000
Wire Wire Line
	3750 2000 3850 2000
Wire Wire Line
	3850 2000 3850 1900
Connection ~ 3750 2000
Wire Wire Line
	3850 2000 3950 2000
Wire Wire Line
	3950 2000 3950 1900
Connection ~ 3850 2000
Wire Wire Line
	3950 2000 4050 2000
Wire Wire Line
	4050 2000 4050 1900
Connection ~ 3950 2000
Wire Wire Line
	4050 2000 4150 2000
Wire Wire Line
	4150 2000 4150 1900
Connection ~ 4050 2000
Text GLabel 1750 1400 1    50   BiDi ~ 0
C-D0
Text GLabel 1850 1400 1    50   BiDi ~ 0
C-D1
Text GLabel 1950 1400 1    50   BiDi ~ 0
C-D2
Text GLabel 2050 1400 1    50   BiDi ~ 0
C-D3
Text GLabel 2150 1400 1    50   BiDi ~ 0
C-D4
Text GLabel 2250 1400 1    50   BiDi ~ 0
C-D5
Text GLabel 2350 1400 1    50   BiDi ~ 0
C-D6
Text GLabel 2450 1400 1    50   BiDi ~ 0
C-D7
Text GLabel 2550 1400 1    50   BiDi ~ 0
C-DP
Text GLabel 2650 1400 1    50   BiDi ~ 0
GND
Text GLabel 2750 1400 1    50   BiDi ~ 0
GND
Text GLabel 2850 1400 1    50   BiDi ~ 0
GND
Text GLabel 3050 1400 1    50   BiDi ~ 0
GND
Text GLabel 3150 1400 1    50   BiDi ~ 0
GND
Text GLabel 3250 1400 1    50   BiDi ~ 0
C-ATN
Text GLabel 3450 1400 1    50   BiDi ~ 0
C-BSY
Text GLabel 3550 1400 1    50   BiDi ~ 0
C-ACK
Text GLabel 3650 1400 1    50   BiDi ~ 0
C-RST
Text GLabel 3750 1400 1    50   BiDi ~ 0
C-MSG
Text GLabel 3850 1400 1    50   BiDi ~ 0
C-SEL
Text GLabel 3950 1400 1    50   BiDi ~ 0
C-C_D
Text GLabel 4050 1400 1    50   BiDi ~ 0
C-REQ
Text GLabel 4150 1400 1    50   BiDi ~ 0
C-I_O
NoConn ~ 2950 1900
NoConn ~ 3350 1400
Text Notes 1550 2250 0    50   ~ 0
SCSI Ribbon Cable
Wire Notes Line
	1400 950  4650 950 
Wire Notes Line
	4650 950  4650 2250
Wire Notes Line
	4650 2250 1400 2250
Wire Notes Line
	1400 2250 1400 950 
$Comp
L power:GND #PWR015
U 1 1 5FED2FC2
P 4150 2000
F 0 "#PWR015" H 4150 1750 50  0001 C CNN
F 1 "GND" H 4155 1827 50  0000 C CNN
F 2 "" H 4150 2000 50  0001 C CNN
F 3 "" H 4150 2000 50  0001 C CNN
	1    4150 2000
	1    0    0    -1  
$EndComp
Connection ~ 4150 2000
Text GLabel 2950 1400 1    50   BiDi ~ 0
TERMPOW
$Comp
L Connector_Generic:Conn_02x25_Odd_Even J9
U 1 1 5FED7B7F
P 3000 3300
F 0 "J9" H 3050 1875 50  0000 C CNN
F 1 "Conn_02x25_Odd_Even" H 3050 1966 50  0000 C CNN
F 2 "Connector_PinHeader_2.00mm:PinHeader_2x25_P2.00mm_Horizontal" H 3000 3300 50  0001 C CNN
F 3 "https://datasheet.lcsc.com/szlcsc/1811151534_BOOMELE-Boom-Precision-Elec-C3407_C3407.pdf" H 3000 3300 50  0001 C CNN
F 4 "C3407" H 3000 3300 50  0001 C CNN "LCSC"
F 5 "CUT DOWN TO 50 PIN - Pin Header 80 2 right-angle，90degrees 2.00mm Through Hole Pin Header" H 3000 3300 50  0001 C CNN "Description"
F 6 "BOOMELE(Boom Precision Elec)" H 3000 3300 50  0001 C CNN "Manufacturer_Name"
F 7 "C3407" H 3000 3300 50  0001 C CNN "Manufacturer_Part_Number"
F 8 "" H 3000 3300 50  0001 C CNN "Mouser Part Number"
F 9 "" H 3000 3300 50  0001 C CNN "Mouser Price/Stock"
	1    3000 3300
	0    1    1    0   
$EndComp
Text GLabel 3500 3600 3    50   BiDi ~ 0
C-D0
Text GLabel 3400 3600 3    50   BiDi ~ 0
C-D1
Text GLabel 3300 3600 3    50   BiDi ~ 0
C-D2
Text GLabel 3200 3600 3    50   BiDi ~ 0
C-D3
Text GLabel 3100 3600 3    50   BiDi ~ 0
C-D4
Text GLabel 3000 3600 3    50   BiDi ~ 0
C-D5
Text GLabel 2900 3600 3    50   BiDi ~ 0
C-D6
Text GLabel 2800 3600 3    50   BiDi ~ 0
C-D7
Text GLabel 2700 3600 3    50   BiDi ~ 0
C-DP
Text GLabel 1900 3100 1    50   BiDi ~ 0
GND
Text GLabel 1900 3600 3    50   BiDi ~ 0
GND
Text GLabel 3600 3600 3    50   BiDi ~ 0
GND
Text GLabel 2500 3100 1    50   BiDi ~ 0
C-ATN
Text GLabel 2500 3600 3    50   BiDi ~ 0
C-BSY
Text GLabel 2400 3600 3    50   BiDi ~ 0
C-ACK
Text GLabel 2300 3100 1    50   BiDi ~ 0
C-RST
Text GLabel 2300 3600 3    50   BiDi ~ 0
C-MSG
Text GLabel 2200 3600 3    50   BiDi ~ 0
C-SEL
Text GLabel 2100 3600 3    50   BiDi ~ 0
C-C_D
Text GLabel 2000 3600 3    50   BiDi ~ 0
C-REQ
NoConn ~ 4200 3600
NoConn ~ 4200 3100
Text Notes 1500 4200 0    50   ~ 0
SCSI  PowerBook Ribbon Cable
Wire Notes Line
	1400 2650 4650 2650
Wire Notes Line
	4650 4200 1400 4200
Text GLabel 2600 3600 3    50   BiDi ~ 0
TERMPOW
Text GLabel 4000 3100 1    39   Input ~ 0
PB_SCSIID_1
Text GLabel 4000 3600 3    39   Input ~ 0
PB_SCSIID_2
Text GLabel 3900 3100 1    39   Input ~ 0
PB_SCSIID_4
NoConn ~ 3800 3100
NoConn ~ 3800 3600
Wire Notes Line
	3750 2950 3750 3750
Wire Notes Line
	3750 3750 3850 3750
Wire Notes Line
	3850 3750 3850 2950
Wire Notes Line
	3850 2950 3750 2950
$Comp
L power:+5V #PWR0101
U 1 1 5FEC7E84
P 3700 3100
F 0 "#PWR0101" H 3700 2950 50  0001 C CNN
F 1 "+5V" V 3700 3300 50  0000 C CNN
F 2 "" H 3700 3100 50  0001 C CNN
F 3 "" H 3700 3100 50  0001 C CNN
	1    3700 3100
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0102
U 1 1 5FEC8724
P 3700 3600
F 0 "#PWR0102" H 3700 3450 50  0001 C CNN
F 1 "+5V" V 3700 3800 50  0000 C CNN
F 2 "" H 3700 3600 50  0001 C CNN
F 3 "" H 3700 3600 50  0001 C CNN
	1    3700 3600
	-1   0    0    1   
$EndComp
$Comp
L power:+5V #PWR0103
U 1 1 5FECCF71
P 1800 3100
F 0 "#PWR0103" H 1800 2950 50  0001 C CNN
F 1 "+5V" V 1800 3300 50  0000 C CNN
F 2 "" H 1800 3100 50  0001 C CNN
F 3 "" H 1800 3100 50  0001 C CNN
	1    1800 3100
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0104
U 1 1 5FECD6DF
P 1800 3600
F 0 "#PWR0104" H 1800 3450 50  0001 C CNN
F 1 "+5V" V 1800 3800 50  0000 C CNN
F 2 "" H 1800 3600 50  0001 C CNN
F 3 "" H 1800 3600 50  0001 C CNN
	1    1800 3600
	-1   0    0    1   
$EndComp
NoConn ~ 2900 3100
Wire Notes Line
	2850 2950 2850 3350
Wire Notes Line
	2850 3350 2950 3350
Wire Notes Line
	2950 3350 2950 2950
Wire Notes Line
	2950 2950 2850 2950
Text GLabel 2100 3100 1    50   BiDi ~ 0
C-I_O
Text GLabel 2000 3100 1    50   BiDi ~ 0
GND
Text GLabel 2200 3100 1    50   BiDi ~ 0
GND
Text GLabel 2400 3100 1    50   BiDi ~ 0
GND
Text GLabel 2600 3100 1    50   BiDi ~ 0
GND
Text GLabel 2700 3100 1    50   BiDi ~ 0
GND
Text GLabel 2800 3100 1    50   BiDi ~ 0
GND
Text GLabel 3000 3100 1    50   BiDi ~ 0
GND
Text GLabel 3100 3100 1    50   BiDi ~ 0
GND
Text GLabel 3200 3100 1    50   BiDi ~ 0
GND
Text GLabel 3300 3100 1    50   BiDi ~ 0
GND
Text GLabel 3400 3100 1    50   BiDi ~ 0
GND
Text GLabel 3500 3100 1    50   BiDi ~ 0
GND
Text GLabel 3600 3100 1    50   BiDi ~ 0
GND
Wire Notes Line
	4650 2650 4650 4200
Wire Notes Line
	1400 2650 1400 4200
Text Notes 1450 2650 0    39   ~ 0
http://ps-2.retropc.se/ppc850tp/scsi-laptop-drive_dprs_spw.pdf
Text GLabel 3900 3600 3    39   Input ~ 0
PB_LED_Pin
NoConn ~ 4100 3100
NoConn ~ 4100 3600
Text Notes 2900 4150 0    79   ~ 16
Remove pins 9, 10 and 27
NoConn ~ 6200 1750
Text GLabel 6200 1650 0    50   BiDi ~ 0
GND
$Comp
L power:+5V #PWR053
U 1 1 607432AF
P 6200 1450
F 0 "#PWR053" H 6200 1300 50  0001 C CNN
F 1 "+5V" V 6200 1650 50  0000 C CNN
F 2 "" H 6200 1450 50  0001 C CNN
F 3 "" H 6200 1450 50  0001 C CNN
	1    6200 1450
	0    -1   -1   0   
$EndComp
Text GLabel 6200 1550 0    50   BiDi ~ 0
GND
Text Notes 4950 2250 0    39   ~ 0
"Floppy" style "berg" power connector
Wire Notes Line
	4950 2250 4950 950 
Wire Notes Line
	7800 2250 4950 2250
Wire Notes Line
	7800 950  7800 2250
Wire Notes Line
	4950 950  7800 950 
$Comp
L SamacSys_Parts:171826-4 J20
U 1 1 607408EC
P 6200 1450
F 0 "J20" H 6828 1346 50  0000 L CNN
F 1 "171826-4" H 6828 1255 50  0000 L CNN
F 2 "SHDRRA4W69P0X250_1X4_1000X1350X670P" H 6850 1550 50  0001 L CNN
F 3 "https://componentsearchengine.com/Datasheets/2/171826-4.pdf" H 6850 1450 50  0001 L CNN
F 4 "Headers & Wire Housings 4P HDR R/A" H 6850 1350 50  0001 L CNN "Description"
F 5 "6.7" H 6850 1250 50  0001 L CNN "Height"
F 6 "571-1718264" H 6850 1150 50  0001 L CNN "Mouser Part Number"
F 7 "https://www.mouser.co.uk/ProductDetail/TE-Connectivity/171826-4/?qs=4hg9R9g9MDt3qYFe2l2pbA%3D%3D" H 6850 1050 50  0001 L CNN "Mouser Price/Stock"
F 8 "TE Connectivity" H 6850 950 50  0001 L CNN "Manufacturer_Name"
F 9 "171826-4" H 6850 850 50  0001 L CNN "Manufacturer_Part_Number"
	1    6200 1450
	1    0    0    -1  
$EndComp
$EndSCHEMATC
